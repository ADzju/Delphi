unit stat2022;    //Модуль процедур статобработки данных

interface
  uses windows,Dialogs,obrotk2022,Classes,SysUtils,Math, StrUtils, Graphics;

const Max_Start=1e306;

var SRmin,SRmax:double;
    bCopyBest: boolean;
    dMinimumAll,T_prirab: double;
    iMaxBest,Ngis: integer;
...
Procedure hist(var Histogr: Hist40); //*************
//Procedure Hist_Made(var Model_Mas:Mod_Weib,  N_Nakopl,SKO_:double);
//Procedure Hist_Made(Model_Hist:Hist40; var Mas_Model_Hist:Mod_Weib; var N_Nakopl,SKO_:double);
//Procedure Hist_Made(var Mas_Model_Hist:Mod_Weib; var N_Nakopl,SKO_:double);
Procedure Hist_Made(A,B:double;Kpowt:double; var Mas_Model_Hist:DoubArr; var N_Nakopl,SKO_:double);
Procedure Kolmogor(Kpwt,A,B,Mu,Sig:double; var Kolm:double;var S:string);
Procedure Sigmas(var Wei:Mod_Weib);

implementation
uses Out_Graph_2_2015;
...
// wcdf :=1-exp(-A*Power(t,b));  //- как в Matlab. А-величина, обратная классическому к-ту масштада
 wf :=exp(-Power(t/B,A))*A/Power(B,A)*Power(t,(A-1));  // как в Excel
 if wf>0 then Result:=wf
           else Result:=1e-305;
           {$I+}
        end;
...
begin
  SS:=0;
with Wei do begin
        LamStart:= 1000*WeibF(A,B,T_Prirab)*K_powtor;
        if MasProbl[High(MasProbl)].time>1000 then
          Lam1000:= 1000*WeibF(A,B,1000)*K_powtor/(1-CDF_Gauss(Sred,SKO,1000))
          else Lam1000:= 1000*WeibF(A,B,MasProbl[High(MasProbl)].time)*K_powtor/(1-CDF_Gauss(Sred,SKO,MasProbl[High(MasProbl)].time));
for i:=0 to high(MasProbl)-1 do begin
    t1:=MasProbl[i].time;
    t2:=MasProbl[i+1].time;
    Fnorm:=(CDF_Gauss(Sred,SKO,(t1+t2)/2));
    FWeib:=weibCDF2(Wei.A,Wei.B,t1,t2);
    Npriv:=Izd_Prived*Wei.K_powtor*FWeib*(1-Fnorm);
      SS:=SS+Npriv;
    Wei.SKO_out:=Wei.SKO_out+Sqr(SS-NPriv);
        end;
...
for i:=0 to high(Hist_Stol) do begin   // предварительно для каждого столбца гистограммы
 x2:=Hist_Stol[i].time;
 MGenet.F_norm[i]:=CDF_Gauss(Mu,Sig,(x1+x2)/2);   // накопление выхода из наблюдения (цензурирования)
   ...
   x1:=0;
   for i:=0 to 30 do      //по столбцам гистограммы
          begin
  x2:=Hist_Stol[i].time;
  ...
  MGenet.SKO_out:=MGenet.SKO_out+Sqr((Hist_Stol[i].Sum-MGenet.Kol_F_Priv[i]){/(i+1)});
  Delta2:=Delta2+Sqr((Hist_Stol[i].Sum-MGenet.Kol_F_Priv[i]))*MGenet.F_Weib[i];
     x1:=x2;
           end;
...
//  SizePopulation :=2*(SizePopulation div 2);
 ...
 MGenet.Kol_F_Priv[i]:=Izd_Prived*MGenet.K_powtor*MGenet.F_Weib[i]*(1-MGenet.F_norm[i]); //приведенное число отказов модели в столбце
    ...
    ListPopulation.Free;
    ListGens.Free;
   Hist_Made(MGenet.A,MGenet.B,MGenet.K_powtor,Kol_Nakopl_Like,MGenet.SKO_out);
     DiaStart;
              end;
                 end;
...
Menshe:
     for I := 0 to 21 do
     for j:=0 to 30 do Histogr[j].Npr[i]:=0;

Ngis:=1;                // число столбцов гистограммы
           for i := 0 to 10 do begin
           Sum_PrichCol[i]:=0; Sum_PrichRow[i]:=false;
           end;
i := 0;  j:=0;  k:=0;   x2:=0;
         repeat    // 111
    ...
    //                        else
    Histogr[k].Npr[round(MasProbl[j].chey)]:=Histogr[k].Npr[round(MasProbl[j].chey)]+1;
                          inc(j);
                      end;
           Histogr[k].Sum:=j-i; //общее число отказов в интервале
           until (j-i>0) or (x2>=MasProbl[high(MasProbl)].time);  //222    true=>выход
        Histogr[k].time:=x2;
        inc(Ngis);
         for I2 := 1 to Mx_prich+1 do Sum_PrichCol[i2]:=Sum_PrichCol[i2]+Histogr[k].Npr[i2];
              inc(k);
         i:=j;
         // inc(Num);
     until x2>=MasProbl[high(MasProbl)].time; // 111
  if (j>70) and (NGis<10) then
   begin D_Hist1:= D_Hist1 div 2;
           goto Menshe;
            end;
if NGis>30 then begin
        D_Hist2:= 2*D_Hist2;
            goto Menshe;
            end;
         end;
...
Var
i,n:integer;
x1,x2,Razn:double;            //плотность распределения - закон Вейбулла
//Razn:RealArray;
  begin
 // with Mas_Model_Hist do
  begin
 n:=Ngis+1;//high(Hist_Arr)+1;                   {число столбцов гистограммы}

 setlength(Mas_Model_Hist,n); // - по числу столбцов
 //setlength(Razn,n);
         x1:=0; N_Nakopl:=0; SKO_:=0;
  for i:=0 to n {high(Hist_Arr)} do begin    // цикл по столбцам 7777
         x2:=Hist_Stol[i].time;
...
//MGenet.Kol_F_Priv[i]:=Izd_Prived*k_pwt*MGenet.F_Weib[i]*(1-MGenet.F_norm[i]);
   N_Nakopl:=N_Nakopl+Mas_Model_Hist[i];
    Razn:=Hist_Stol[i].Sum-Mas_Model_Hist[i];
   SKO_:=SKO_+Sqr(Razn{/high(Mas_Otkazov[1])});
      x1:=x2;
    end;
 SKO_:=Sqrt(SKO_/n);
...
begin
  ...
  for i:=0 to high(MasProbl) do
    ...
    if Sum_PrichRow[i1] then x2:=x2+1/(1-CDF_Gauss(Sred,SKO,x1));
  end;
...
var Delta2,x1,x2,Kritery_Moy,P_sum,Kol: double;
  ...
  begin
  x1:=0;
  for i:=0 to NGis do begin
       x2:=Hist_Stol[i].time;
       if i>0 then x1:=Hist_Stol[i-1].time; // dec 2015
   MPir.F_norm[i]:=CDF_Gauss(Mu,Sig,(x2+x1)/2); // dec 2015  // накопленная доля выбывших по столбцам
    ...
    Kol:=Izd_Prived*k_pwt; //Kol_Izw                         // считаем количество известных
      x1:=0;
 for i:=0 to 30 {high(Hist_Arr)} do begin                         // по столбцам гистограммы
  x2:=Hist_Stol[i].time;
  ...
  if Form1.RadioGroup1.ItemIndex=0
     ...
     else Delta2:=Delta2+Sqr((Hist_Stol[i].Sum-MPir.Kol_F_Priv[i])*MPir.F_Weib[i]);
  x1:=x2;
 end;
...
Met:
 ...
 MPirsN.Unsens:=0; x1:=0;
    for i:=0 to 30 {high(Hist_Arr)} do begin
      x2:=Hist_Stol[i].time;
